...
class LedDisplayEmulatorTask(BaseActuatorSimTask):
    """
    Emulator for LED Display Actuator, simulating activation and deactivation.
    """
    ...
    def __init__(self):
        ...
        # Initialize the SenseHAT with emulation flag
    ...
    def _activateActuator(self, val: float = ConfigConst.DEFAULT_VAL, stateData: str = None) -> int:
        if self.sh.screen:
            if stateData:  # Check if stateData is provided
                self.sh.screen.scroll_text(stateData, size=8)
                return 0
            else:
                logging.warning("No state data provided to scroll.")
                return -1
        else:
