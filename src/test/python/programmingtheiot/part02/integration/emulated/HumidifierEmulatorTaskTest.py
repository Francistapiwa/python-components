...
class HumidityEmulatorTaskTest(unittest.TestCase):
    ...
    def testReadEmulator(self):
        ...
        if sd1:
            ...
            logging.info("SensorData: %f - %s", sd1.get_value(), str(sd1))  # Correct usage of get_value()
            sleep(5)
        else:
        ...
        if sd2:
            ...
            logging.info("SensorData: %f - %s", sd2.get_value(), str(sd2))  # Correct usage of get_value()
            sleep(5)
        else:
            logging.warning("FAIL: SensorData is None.")
